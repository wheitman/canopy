...
export const TexturePainter = function (renderer, camera, mesh, src) {
  ...
  this.update = function () {
    if (!scope.camera.position.equals(cameraPosition)) cameraUpdated = true;
  ...
  var initialize = (function (src) {
    ...
    scope.texture =
      scope.mesh.material.map || new THREE.Texture(undefined, THREE.UVMapping);
    ...
    scope.ortho = new THREE.OrthographicCamera(
  ...
  function faceClip(clip) {
    scope.ctx.beginPath();
    scope.ctx.moveTo(
      clip[0].x * scope.canvas.width,
      clip[0].y * scope.canvas.height
    );
    scope.ctx.lineTo(
      clip[1].x * scope.canvas.width,
      clip[1].y * scope.canvas.height
    );
    scope.ctx.lineTo(
      clip[2].x * scope.canvas.width,
      clip[2].y * scope.canvas.height
    );
    scope.ctx.closePath();
    scope.ctx.clip();
  }
  ...
  function getClipVertex(vertexID) {
    if (verticesDict[vertexID])
      return verticesDict[vertexID].clone().sub(scope.reference);

    verticesDict[vertexID] = calculateClipVertex(
      scope.mesh.geometry.vertices[vertexID]
    );
  ...
  function getDrawLocations() {
    ...
    var origin = new THREE.Vector3().setFromMatrixPosition(
      scope.camera.matrixWorld
    );
